...
GITHUB_REPO  = "https://github.com/terminaldidy115/yuba-county-flock-oversight.git"
REPO_DIR     = Path("/Users/thomashammock/Desktop/Flock")
# ─────────────────────────────────────────────────────────────────────────────
...
def download_csv() -> pd.DataFrame:
    with tempfile.TemporaryDirectory() as tmp_dir:
        with sync_playwright() as p:
            browser = p.chromium.launch(headless=False)  # headless=False required for Flock portal JS rendering
...
def push_to_github(run_time: datetime, added: int):
    """Commit updated files in the Desktop/Flock repo and push."""
    commit_msg = (
        f"Auto-update {run_time:%Y-%m-%d %H:%M} — {added} new row(s)"
        if added > 0
        else f"Auto-update {run_time:%Y-%m-%d %H:%M} — no new rows"
    )
    subprocess.run(["git", "-C", str(REPO_DIR), "add",
                    "Master Audit.xlsx"], check=True)
    ...
    if diff.returncode == 0:
        print("  GitHub: nothing changed, skipping push")
        return
    ...
    print(f"  GitHub push complete → {GITHUB_REPO}")
...
def main():
    # Prevent display sleep for up to 5 minutes while the script runs
    caffeinate = subprocess.Popen(["caffeinate", "-d", "-t", "300"])
