...
MASTER_PATH  = Path("/Users/thomashammock/Flock/Master Audit.xlsx")
SHEET_NAME   = "Sheet1"
ID_COLUMN    = "id"
...
def backup_master(path: Path) -> Path:
    ...
    backup = path.parent / "Master Backup" / f"Master Audit BACKUP {timestamp}.xlsx"
...
if __name__ == "__main__":
    ...
    subprocess.run([
        "git", "-C", REPO,
        "add", "Master Audit.xlsx", "search_audit_pacific.xlsx", "Reports/flock_run_log.csv"
    ])
    subprocess.run([
        "git", "-C", REPO,
        "commit", "-m", f"Daily update {datetime.now().strftime('%Y-%m-%d')}"
    ])
    subprocess.run([
        "git", "-C", REPO,
        "pull", "--rebase"
