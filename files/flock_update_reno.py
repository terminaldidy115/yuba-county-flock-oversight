...
merges new rows into Master Audit Reno.xlsx, and saves a run report CSV.

Usage:
    python3 flock_update_reno.py
...
from flock_email_reporter_reno import send_flock_report

# ── CONFIG ────────────────────────────────────────────────────────────────────
FLOCK_URL    = "https://transparency.flocksafety.com/reno-nv-pd"
MASTER_PATH  = Path("/Users/thomashammock/Flock/Master Audit Reno.xlsx")
SHEET_NAME   = "Reno Master Audit"
ID_COLUMN    = "id"
DATE_COLUMN  = "searchDate"
REASON_COL   = "reason"
REPORTS_DIR  = Path("/Users/thomashammock/Flock/Reports")
RUN_LOG_PATH = REPORTS_DIR / "flock_run_log_reno.csv"
# ─────────────────────────────────────────────────────────────────────────────


def backup_master(path: Path) -> Path:
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup = path.parent / "Master Backup Reno" / f"Master Audit Reno BACKUP {timestamp}.xlsx"
...
if __name__ == "__main__":
    ...
    subprocess.run([
        "git", "-C", REPO,
        "add", "Master Audit Reno.xlsx", "Reports/flock_run_log_reno.csv"
    ])
    subprocess.run([
        "git", "-C", REPO,
        "commit", "-m", f"Daily Reno update {datetime.now().strftime('%Y-%m-%d')}"
    ])
