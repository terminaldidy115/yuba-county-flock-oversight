...
merges new rows into Master Audit Lincoln.xlsx, and saves a run report CSV.

Usage:
    python3 flock_update_lincoln.py
...
from flock_email_reporter_lincoln import send_flock_report

# ── CONFIG ────────────────────────────────────────────────────────────────────
FLOCK_URL    = "https://transparency.flocksafety.com/lincoln-ca-pd"
MASTER_PATH  = Path("/Users/thomashammock/Flock/Master Audit Lincoln.xlsx")
SHEET_NAME   = "Master Audit Lincoln"
ID_COLUMN    = "id"
DATE_COLUMN  = "searchDate"
REASON_COL   = "reason"
REPORTS_DIR  = Path("/Users/thomashammock/Flock/Reports")
RUN_LOG_PATH = REPORTS_DIR / "flock_run_log_lincoln.csv"
# ─────────────────────────────────────────────────────────────────────────────


def backup_master(path: Path) -> Path:
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup = path.parent / "Master Backup Lincoln" / f"Master Audit Lincoln BACKUP {timestamp}.xlsx"
...
if __name__ == "__main__":
    ...
    REPO = "/Users/thomashammock/Flock"
    subprocess.run([
        "git", "-C", REPO,
        "add", "Master Audit Lincoln.xlsx", "Reports/flock_run_log_lincoln.csv"
    ])
    subprocess.run([
        "git", "-C", REPO,
        "commit", "-m", f"Daily Lincoln update {datetime.now().strftime('%Y-%m-%d')}"
    ])
